...
function StatCard({ icon, label, value, sub, color }) {
    return (
        <div style={{ background: '#111', border: '1px solid rgba(245,168,0,0.15)', borderRadius: '16px', padding: '1.25rem', borderLeft: `4px solid ${color}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: `${color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', color, flexShrink: 0 }}>
                    <i className={`fas ${icon}`} />
                </div>
                <span style={{ color: '#444', fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'right', lineHeight: 1.3 }}>{label}</span>
            </div>
            <div style={{ fontSize: '1.75rem', fontWeight: 900, color: '#fff', marginBottom: '0.2rem', wordBreak: 'break-all' }}>{value}</div>
            {sub && <div style={{ color: '#666', fontSize: '0.78rem' }}>{sub}</div>}
        </div>
    );
}

const grid4 = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: '1rem', marginBottom: '1.5rem' };
const grid2 = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(320px, 100%), 1fr))', gap: '1.25rem', marginBottom: '1.5rem' };
    ...
    return (
        <AdminLayout admin={admin} active="dashboard">
            <div style={{ marginBottom: '1.5rem' }}>
                <h1 style={{ color: '#fff', fontSize: '1.4rem', fontWeight: 800, margin: 0 }}>Dashboard</h1>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '150px' }}>
                        {chartData?.map((d, i) => (
                            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', height: '100%', justifyContent: 'flex-end', minWidth: 0 }}>
                                <div style={{ width: '100%', borderRadius: '4px 4px 0 0', background: d.total > 0 ? 'linear-gradient(180deg,#f5a800,#cc0000)' : '#1a1a1a', height: `${(d.total / maxChart) * 130}px`, minHeight: '4px' }} />
                                <span style={{ color: '#444', fontSize: '0.55rem' }}>{d.month}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div style={{ background: '#111', border: '1px solid rgba(245,168,0,0.15)', borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(245,168,0,0.15)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
                        ...
                        <thead>
                            <tr style={{ borderBottom: '1px solid #1a1a1a' }}>
                                {['Order #', 'Customer', 'Amount', 'Status', 'Payment', 'Date'].map(h => (
                                    <th key={h} style={{ padding: '0.75rem 1rem', color: '#555', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left', whiteSpace: 'nowrap' }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {recentOrders?.length > 0 ? recentOrders.map(order => (
                                <tr key={order.id} style={{ borderBottom: '1px solid #161616' }}
                                    ...
                                    <td style={{ padding: '0.85rem 1rem', color: '#f5a800', fontWeight: 700, fontSize: '0.85rem', whiteSpace: 'nowrap' }}>#{order.order_number}</td>
                                    <td style={{ padding: '0.85rem 1rem', color: '#fff', fontSize: '0.85rem', maxWidth: '140px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{order.user?.name || 'N/A'}</td>
                                    <td style={{ padding: '0.85rem 1rem', color: '#fff', fontWeight: 700, fontSize: '0.85rem', whiteSpace: 'nowrap' }}>{fmt(order.total)}</td>
                                    <td style={{ padding: '0.85rem 1rem', whiteSpace: 'nowrap' }}>
                                        <span style={{ padding: '0.25rem 0.65rem', borderRadius: '50px', background: `${statusColors[order.status]}22`, color: statusColors[order.status], fontSize: '0.72rem', fontWeight: 700, textTransform: 'capitalize' }}>{order.status}</span>
                                    </td>
                                    <td style={{ padding: '0.85rem 1rem', whiteSpace: 'nowrap' }}>
                                        <span style={{ padding: '0.25rem 0.65rem', borderRadius: '50px', background: order.payment_status === 'paid' ? '#22c55e22' : '#ef444422', color: order.payment_status === 'paid' ? '#22c55e' : '#ef4444', fontSize: '0.72rem', fontWeight: 700, textTransform: 'capitalize' }}>{order.payment_status}</span>
                                    </td>
                                    <td style={{ padding: '0.85rem 1rem', color: '#666', fontSize: '0.78rem', whiteSpace: 'nowrap' }}>{new Date(order.created_at).toLocaleDateString('en-IN')}</td>
                                </tr>
                            )) : (
                                <tr>
